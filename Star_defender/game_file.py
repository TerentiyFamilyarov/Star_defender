...
from PyQt6.QtCore import QPoint, QRectF, Qt, QTimer, QRect, pyqtSignal, QThread, pyqtSlot
from PyQt6.QtGui import QColor, QPainter, QBrush, QFont, QPixmap, QPen, QPainterPath
from PyQt6.QtWidgets import QApplication, QGraphicsRectItem, QGraphicsView, QGraphicsScene, QWidget, \
...
class SceneGame(QGraphicsScene):
    ...
    def update_scene(self, player, array_enemies, array_bullets):
        ...
        if enemies_file.timer.isActive() is False and len(enemies_file.array_enemies) < 20:
            type_enemy = random.choice(self.available_enemy_types)
            enemy = type_enemy(self)
            enemies_file.timer.start(60)
        player.move()
    ...
    def draw_time_timer_text(self):
        ...
        if self.sec < 10:
            ...
            text += f'{self.sec}:'
        if int(self.msec) < 10:
            text += f'0{int(self.msec)}'
        elif int(self.msec) > 99:
            self.game_sec_time += 1
            self.sec += 1
            self.msec = 0
        else:
            text += f'{int(self.msec)}'
